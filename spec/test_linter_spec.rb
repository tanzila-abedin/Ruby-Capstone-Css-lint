...
describe Linters do
  ...
  describe 'font_check' do
    it 'check for invalid input as font weight' do
      result = check.send(:font_check, '/\d/i', 47)
      expect(result).not_to eql(['put a valid name or number as font-weight'])
    end
    it 'check for no invalid input as font weight' do
      result = check.send(:font_check, '/\d/i', 47)
      expect(result).to eql(nil && false)
    end
  end
